...
| 📤 **邀请** | 一键复制活动邀请文案发到群里；朋友点开后先看活动概览，加入圈子后解锁完整筹备 |
| 👥 **好友** | 登录过的朋友自动同步，手动添加的好友可绑定真实账号 |
| 📤 **分享** | 活动/海报可转发，圈外人只看到概览（标题/时间/地点/人数），成员名单与筹备内容不泄露；圈子链接不含通行码，须凭码加入 |

---

## 🛠 技术栈

- **前端**：微信原生小程序（WXML / WXSS / JS），深空霓虹主题（深蓝夜空 + 微网格 + 光晕）+ 深色玻璃拟态 + SVG 线性图标
...
**Q：转发出去，别人能看到什么？**
A：只看到**活动概览**（标题、时间、地点、参与人数、圈名）。成员名单、照片、筹备进度、签到、通行码等一律不对外展示；加入圈子后才能解锁完整详情。圈子分享链接本身不含通行码，通行码建议私聊发送给信任的朋友。
