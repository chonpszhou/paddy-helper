    ...
    query.select('#posterCanvas').fields({ node: true, size: true }).exec(function (res) {
      ...
      if (self.data.aiBgPath) {
        ...
        img.onload = function () {
          ...
          const topShade = ctx.createLinearGradient(0, 0, 0, HERO_H)
          topShade.addColorStop(0, 'rgba(255,255,255,0.62)')
          topShade.addColorStop(1, 'rgba(255,255,255,0)')
          ctx.fillStyle = topShade
          ctx.fillRect(0, 0, W, HERO_H + 40)
          drawAll()
          finish()
        }
