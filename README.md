...
| 🍲 **聚餐** | 家里 / 餐厅双模式；家里模式：时间投票 + 点菜 + 带菜登记 + **自动生成采购清单**；餐厅模式免带菜免采购 |
| 🎁 **私房菜彩蛋** | 地点是「Paddy家」自动解锁 **50 道北方私房菜单**（鄂尔多斯蒙式 + 巴彦淖尔河套 + 东北硬菜） |
| ⛺ **户外** | 拼车登记与座位自动安排、装备清单认领、**按活动地点查询当地天气**与出行提醒 |
| 🎲 **团体活动** | 按参加者位置**自动计算居中地点**并在地图上展示，候选场馆投票与选定；狼人杀局角色分配 + 上帝管理 |
| ✈️ **旅行** | 多日行程规划、分工认领、住宿登记、AA 预算记账 |
| 🎨 **海报** | 苹果风极光主题长图海报一键生成，分享给朋友点开直达活动 |
| 🔔 **通知** | 订阅消息：创建活动自动提醒圈内好友，点击直达详情 |
| 📍 **签到** | 活动当天报名者一键签到，谁到了谁没到一目了然 |
| ✨ **智能推荐** | 聚餐人气时间/人气菜、户外天气建议、团体居中地点（纯数据统计，合规） |
| 📤 **邀请** | 一键复制活动邀请文案（含圈子通行码），发到微信群即可拉人 |
...
| `weather` | 天气转发（按活动地点查询，避免生产环境域名限制） |

> 仓库里的 `ai`、`h5api` 云函数服务于网页版（h5 落地页/小程序外体验），小程序本体不依赖它们；不部署也不影响小程序功能。
